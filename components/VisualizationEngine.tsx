...
const OrbVisualizer = ({ audio }: { audio: MutableRefObject<AudioBands> }) => {
  ...
  useFrame(({ clock }) => {
    ...
    if (glowRef.current) {
      ...
      (glowRef.current.material as THREE.MeshBasicMaterial).opacity = 0.35 + energy * 0.45;
    }
  ...
  return (
    <group>
      ...
      <mesh ref={glowRef}>
        ...
        <meshBasicMaterial color="#ffbf7a" transparent opacity={0.5} blending={THREE.AdditiveBlending} />
      </mesh>
      <points ref={particlesRef}>
        <bufferGeometry>
          <bufferAttribute attach="attributes-position" array={particles} count={particles.length / 3} itemSize={3} />
        </bufferGeometry>
        <pointsMaterial color="#b057ff" size={0.06} opacity={0.7} transparent depthWrite={false} />
      </points>
    </group>
  );
};
...
const NeonBarsVisualizer = ({ audio, lowQuality }: { audio: MutableRefObject<AudioBands>; lowQuality?: boolean }) => {
  ...
  useFrame(() => {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      tempObject.position.set(Math.cos(angle) * radius, Math.sin(angle) * radius, -0.4 + Math.sin(angle * 2) * 0.3);
  ...
  return (
    <instancedMesh ref={meshRef} args={[undefined, undefined, count]}>
      ...
      <meshStandardMaterial vertexColors emissive="#7af7ff" emissiveIntensity={0.6} roughness={0.2} />
    </instancedMesh>
  );
};
...
const ParticleFieldVisualizer = ({
  ...
  return (
    <Points ref={pointsRef} positions={positions} stride={3}>
      <PointMaterial color="#7af7ff" size={0.05} sizeAttenuation depthWrite={false} transparent opacity={0.8} />
    </Points>
  );
};

// Glowing wireframe lattice that distorts on bass/highs for subtle glitch.
const WireframeLatticeVisualizer = ({ audio }: { audio: MutableRefObject<AudioBands> }) => {
...
const WaveformTunnelVisualizer = ({ audio }: { audio: MutableRefObject<AudioBands> }) => {
...
const HolographicRingsVisualizer = ({ audio }: { audio: MutableRefObject<AudioBands> }) => {
  const groupRef = useRef<THREE.Group>(null);
    ...
    <group ref={groupRef}>
      {rings.map((ring) => (
        <mesh key={ring} position={[0, 0, ring * -0.1]}>
          ...
          <meshBasicMaterial color={NEON_COLORS.cyan} transparent opacity={0.4} blending={THREE.AdditiveBlending} />
        </mesh>
      ))}
    </group>
  );
    ...
    <div className="visualizer-selector" role="tablist" aria-label="Visualizer">
      {Object.entries(MODE_LABELS).map(([key, label]) => (
        <button
          key={key}
          type="button"
          className={`visualizer-selector__button${mode === key ? ' visualizer-selector__button--active' : ''}`}
...
export const VisualizationEngine = ({
  ...
  return (
    <div className={className ?? 'visualization-engine'}>
      <Canvas dpr={lowQuality ? 1 : [1, 2]} gl={{ antialias: !lowQuality, alpha: true }}>
        <PerspectiveCamera makeDefault position={[0, 0, 6]} />
        <color attach="background" args={['#05050d']} />
        <ambientLight intensity={0.4} color={NEON_COLORS.cyan} />
        <directionalLight position={[4, 6, 4]} intensity={0.8} color={NEON_COLORS.gold} />
      ...
      {onModeChange && <VisualizationSelector mode={mode} onChange={onModeChange} />}
    </div>
  );
};
