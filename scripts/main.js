  ...
  if (albumParam && (trackParam || trackIdParam)) {
    ...
    if (albumIndex !== -1) {
      ...
      if (trackIdParam) {
        ...
        if (
          location &&
          Number.isInteger(location.albumIndex) &&
          Number.isInteger(location.trackIndex) &&
          location.albumIndex === albumIndex
        ) {
...
async function shareContent({ includeProverbCard = false } = {}) {
  ...
  if (includeProverbCard && window.AriyoProverbUtils) {
    ...
    proverbPayload = formatProverbSharePayload(proverb, buildShareUrl(playbackContext, { includeProverbCard: true }));
  }
...
if (searchInput && searchResults && searchPanel) {
  ...
  const buildTrackEntry = (track, { albumIndex, trackIndex, albumTitle, albumCover } = {}) => {
    ...
    const searchText = normalizeSearchText(
      [title, artist, albumTitle, Array.isArray(track.tags) ? track.tags.join(' ') : '', track.subtitle]
        .filter(Boolean)
        .join(' '),
    );
  ...
  const buildSearchIndex = ({ reason = 'init' } = {}) => {
    ...
    if (Array.isArray(albums)) {
      const provider =
        (trackCatalogApi.getProvider ? trackCatalogApi.getProvider() : null) ||
    ...
    if (Array.isArray(stationList) && stationList.length) {
      stationList.forEach((station, index) => {
        ...
        const searchText = normalizeSearchText(
          [name, station.location, station.region, station.country, station.type, 'radio'].filter(Boolean).join(' '),
        );
    ...
    if (rssAlbums.length) {
      rssAlbums.forEach((album, index) => {
        ...
        const searchText = normalizeSearchText(
          [name, album.artist, album.category, 'podcast', 'rss'].filter(Boolean).join(' '),
        );
  ...
  const findTrackLocationByTitle = (title) => {
    ...
    for (let albumIndex = 0; albumIndex < albums.length; albumIndex += 1) {
      ...
      const trackIndex = album.tracks.findIndex(
        (track) => normalizeSearchText(track?.title || track?.name) === normalizedTitle,
      );
      if (trackIndex >= 0) {
        return { albumIndex, trackIndex };
      }
  ...
  const handleSearchSelection = (result) => {
    ...
    if (result.type === 'track') {
      ...
      if (typeof window.loadFullLibraryData === 'function') {
        window
          .loadFullLibraryData({ reason: 'search-track-select', immediate: true })
          .then(() => {
...
/* BACKGROUND CYCLER */
const backgrounds = ['Naija AI1.png', 'Naija AI2.png', 'Naija AI3.png'];
...
function updateMediaSession() {
  if ('mediaSession' in navigator) {
    ...
    const track = currentRadioIndex === -1 ? album?.tracks?.[currentTrackIndex] : radioStations?.[currentRadioIndex];
    ...
    const artwork = currentRadioIndex === -1 ? track?.cover || album?.cover || fallbackArtwork : track?.logo;

    navigator.mediaSession.metadata = new MediaMetadata({
      title: currentRadioIndex === -1 ? track?.title || fallbackTitle || 'Track' : track.name + ' - ' + track.location,
      artist:
        currentRadioIndex === -1
...
function initializePlayer() {
  ...
  if (stationParam) {
    ...
    if (stationIndex !== -1) {
      ...
      selectRadio(station.url, `${station.name} - ${station.location}`, stationIndex, station.logo);
      if (typeof attemptPlay === 'function') {
        attemptPlay();
      }
      return;
...
const updateEdgePanelHeight = () => {
  ...
  if (isCompactLayout) {
    ...
    const relaxedGuard = isVeryNarrowViewport
      ...
      : guardTarget;
    const guardPreference = isVeryNarrowViewport ? Math.min(guardTarget, relaxedGuard) : guardTarget;
  ...
  const itemsHeight = visibleCount > 0 ? visibleCount * launcherHeight + Math.max(0, visibleCount - 1) * gapValue : 0;
...
if (sidebarNav) {
  ...
  const sidebarGuard = new MutationObserver(() => {
    ...
    enforceSidebarVisibility();
    sidebarGuard.observe(sidebarNav, {
      attributes: true,
