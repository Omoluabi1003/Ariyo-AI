// Serve audio from CDN/object storage-backed URLs via the API manifest.
(async () => {
  ...
  const hydrate = async () => {
    ...
    const albums = buildAlbumsFromTracks(tracks);
    attachPlaylistAlbum(albums);

    window.albums = albums;
    window.radioStations = normalizeStations(stations);
    window.latestTracks = [];
  ...
  try {
    ...
    console.warn('[catalog] Full catalog load failed:', error);
    window.albums = [];
    window.radioStations = [];
    window.latestTracks = [];
    window.__ariyoLibraryHydrated = false;
    window.__ariyoLibraryMode = 'full';
    window.dispatchEvent(new CustomEvent('ariyo:library-ready', { detail: { source: 'full-failed' } }));
  }
})();
