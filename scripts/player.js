...
const syncLibraryState = ({ source = 'init' } = {}) => {
  ...
  if (window.AriyoTrackCatalog && typeof window.AriyoTrackCatalog.createProvider === 'function') {
    ...
    if (DEBUG_AUDIO) {
      console.debug('[catalog-sync]', { source, tracks: trackCatalogProvider.trackCatalog.length });
    }
  }
...
function getKeepAliveAudio() {
  if (backgroundKeepAlive.audio) {
    return backgroundKeepAlive.audio;
  }
  ...
  backgroundKeepAlive.audio = audio;
  return audio;
}

function startBackgroundKeepAlive(reason = 'playback') {
  if (backgroundKeepAlive.active || userInitiatedPause) {
    return;
  }
  if (!playIntent && playbackStatus !== PlaybackStatus.playing) {
    return;
  }
...
function stopBackgroundKeepAlive() {
  ...
  try {
    backgroundKeepAlive.audio.pause();
    backgroundKeepAlive.audio.currentTime = 0;
  } catch (error) {
...
function setPlayIntent(nextIntent) {
  const resolvedIntent = Boolean(nextIntent);
  if (playIntent === resolvedIntent) return;
  playIntent = resolvedIntent;
  manageVinylRotation();
}
...
function ensureBackgroundPlayback(reason = 'hidden') {
  ...
  if (document.visibilityState === 'hidden') {
    ...
    startBackgroundKeepAlive(`background-${reason}`);
  }
...
function activateOfflineFallback(reason = 'network') {
  ...
  if (trackArtistInline) {
    trackArtistInline.textContent = artist;
  }
  if (trackAlbum) {
    trackAlbum.textContent = 'Album: Offline queue';
  }
  updateTrackThumbnail({ title, artist });
...
function addTrackToPlaylistByIndex(albumIndex, trackIndex) {
  ...
  if (!album || !Array.isArray(album.tracks)) {
    alert('Select a track before adding it to your playlist.');
    return;
  }
  ...
  const playlistAlbum = albums[playlistAlbumIndex];
  const playlist = playlistAlbum?.tracks;
