...
function startTimer(elapsed = 0) {
    ...
    timerEl.textContent = `Time: ${Math.floor(elapsed / 1000)}s`;
...
function saveGameState() {
    const boardData = board.map(row => row.map(cell => ({
        ...
        words: cell.dataset.words || ''
    })));
    const cellsData = {};
    for (const [word, cells] of Object.entries(foundWordCells)) {
        cellsData[word] = cells.map(c => [parseInt(c.dataset.row), parseInt(c.dataset.col)]);
    }
    const state = {
        selectedCategory,
        gridSize,
...
function loadGameState() {
    ...
    wordsInGame = data.wordsInGame;
    foundWords = [];
    foundWordCells = {};
    startTimer(data.elapsedTime || 0);
    createBoard();
    for (let r = 0; r < gridSize; r++) {
        for (let c = 0; c < gridSize; c++) {
            const cell = board[r][c];
            ...
            if (cellData.words) cell.dataset.words = cellData.words;
        }
    }
    populateWordList();
    for (const word of data.foundWords) {
        const coords = data.foundWordCells[word] || [];
