...
// Add tracks from albums.js to the Omoluabi category if available
if (typeof albums !== "undefined") {
    const extraTitles = albums.flatMap(a => a.tracks.map(t => t.title));
    const sanitize = t => t.toLowerCase().replace(/[^a-z]/g, "");
    const extraWords = extraTitles.map(sanitize);
    categories["Omoluabi songs"] = Array.from(new Set([
        ...categories["Omoluabi songs"],
        ...extraWords
    ]));
}
